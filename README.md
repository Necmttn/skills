...
- **[scrapecreators](skills/engineering/scrapecreators/SKILL.md)** - find competitor ads, download and verify their media, and assess credit use through the ScrapeCreators CLI/API.
...
- **[wrap-up](skills/engineering/wrap-up/SKILL.md)** - close down a work session or agent pane: review the work against every dimension (complete, edge cases, correct, consistent, production ready, goals met), land or file everything of value (follow-up tickets), queue human checks on a `uat` checklist issue, safe-only housekeeping, then report reviewed/closed/filed/kept.
- **[fix-all-issues](skills/engineering/fix-all-issues/SKILL.md)** - fix every identified issue in the current working state to production-ready quality (no workarounds, no `TODO`s, no suppressed lints), leaving the changes uncommitted.
