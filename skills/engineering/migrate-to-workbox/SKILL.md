...
4. **Start the successor on the workbox.** `ssh <workbox> 'herdr agent start <name> --cwd <repo-or-worktree> -- <agent-cli> <bypass-flag>'`. Clear the startup trust/hook prompts once it boots. Give each successor its **own labeled workspace** (`--workspace <id>`, or `pane move --new-workspace --label <repo>` after) - never pack agents into one workspace, it makes sessions unreviewable. **Done when: `agent list` on the workbox shows the new pane idle.**
